...
__global__ void emit_coo_values(int * row, int * col, double * values, const Resistor * resistors, int num_resistors) {
    ...
    if (tid < num_resistors) {
        Resistor R = resistors[tid];

        // S_11
        row[4 * tid + 0] = R.node[0];
        col[4 * tid + 0] = R.node[0];
        values[4 * tid + 0] = 1.0 / R.resistance;

        // S_12
        row[4 * tid + 1] = R.node[0];
        col[4 * tid + 1] = R.node[1];
        values[4 * tid + 1] = -1.0 / R.resistance;
