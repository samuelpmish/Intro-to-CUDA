...
__global__ void calculate_normals_kernel(const vec4 * vertices, const vec4i * triangles, vec4 * normals, int n) {
  int i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i < n) {
    vec4i tri = triangles[i];
    vec3 v0 = xyz(vertices[tri[0]]);
    vec3 v1 = xyz(vertices[tri[1]]);
    vec3 v2 = xyz(vertices[tri[2]]);
    vec3 n = normalize(cross(v1 - v0, v2 - v0));
    normals[i] = vec4{n[0], n[1], n[2], 0.0};
  }
}
